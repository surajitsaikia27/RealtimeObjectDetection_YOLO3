...
def detect_frame(yolo, frame):
    ...
    if len(prediction)!=0:
        for i in range(len(prediction)):
            bbox=[]
            bbox.append(prediction[i][0])
            bbox.append(prediction[i][1])
            bbox.append(prediction[i][2])
            bbox.append(prediction[i][3])
            class_name.append(prediction[i][4])
            scores.append(prediction[i][5])
            bbox_all.append(bbox)

    return bbox_all, class_name, scores
